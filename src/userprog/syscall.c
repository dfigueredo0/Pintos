...
void sys_open(struct intr_frame* f) {
  ...
  user_ptr++;
  acquire_lock_f();
  struct file *f_ptr = filesys_open((const char *)*user_ptr);
  release_lock_f();
  
  if (f_ptr != NULL) {
    struct thread_file *tfile = malloc(sizeof(struct thread_file));
    tfile->file = f_ptr;
    tfile->fd = thread_current()->file_fd++;
    list_push_back(&thread_current()->files, &tfile->file_elem);
    f->eax = tfile->fd;
  } else {
    f->eax = -1;
  }
...
void sys_write(struct intr_frame* f) {
  ...
  if (fd == 1) {
    ...
    if (thread_file_temp == NULL) {
      ...
      if (thread_file_temp->file == thread_current()->file_owned) {
        f->eax = 0;
        return;
      }
      acquire_lock_f();
...
sys_read (struct intr_frame* f)
{
  ...
  /* We don't know how to fix the bug, just check the pointer */
  int fd = *user_ptr;
  int i;
...
{
  /*just add 1 to its first argument, and print its result*/
  int * p =f->esp;
  check_ptr2 (p + 1);
  int type = * (int *)f->esp;
